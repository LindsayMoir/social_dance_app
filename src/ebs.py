    ...
    try:
        ...
        query = """
            SELECT DISTINCT
                ...
                day_of_week, event_type, dance_style, description,
        ...
        logging.info(f"export_events_to_csv(): Executing query")
        df = pd.read_sql(text(query), db_handler.conn)
        logging.info(f"export_events_to_csv(): Query returned {len(df)} rows")
        ...
        import traceback
        logging.error(f"export_events_to_csv(): Traceback: {traceback.format_exc()}")
